...
public class RootModel implements IRootModel {
	...
	public List<ExistingGame> getExistingGames(User user) {
		List<ExistingGame> existingGames = new ArrayList<>();
		for (ServerSession session : this.currentSessions.values()) {
			if (session.getUser().getUsername().equals(user.getUsername())) {
				String gameID = session.getGameID();
				if (gameID != null) {
					ActiveGame activeGame = this.activeGames.get(gameID);
					if (activeGame != null) {
						String currentPlayerUsername = activeGame.getCurrentPlayer().getUsername();
						ExistingGame game = new ExistingGame(activeGame.getID(),
															 activeGame.getName(),
															 activeGame.getOwner(),
	...
	public Session getSessionByUsernameAndGameID(String username, String gameId) {
		...
		for (ServerSession session : this.currentSessions.values()) {
			if (session.getUser().getUsername().equals(username) &&
					session.getGameID() != null &&
					session.getGameID().equals(gameId)) {
